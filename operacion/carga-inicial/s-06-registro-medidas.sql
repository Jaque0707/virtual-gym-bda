...
create or replace procedure carga_registro_medidas is
  ...
  v_masa_txt      VARCHAR2(40);
  v_estatura_txt  VARCHAR2(40);
  v_peso_txt      VARCHAR2(40);
...
BEGIN
  DBMS_OUTPUT.PUT_LINE('Leyendo OPERA_DIR/registro_medidas.csv ...');

  v_file := UTL_FILE.FOPEN('OPERA_DIR', 'registro_medidas.csv', 'R', 32767);
  ...
  for i in 1 .. 1000 loop
    ...
    -- Si tu registro_medidas.csv NO tiene encabezado, comenta este bloque
    ...
    v_fecha_txt    := col(v_cols, 1);
    v_masa_txt     := col(v_cols, 2);
    v_estatura_txt := col(v_cols, 3);
    v_peso_txt     := col(v_cols, 4);
    v_cliente_txt  := col(v_cols, 5);

    BEGIN
      INSERT INTO registro_medidas (
        fecha_registro,
        masa_corporal,
        estatura,
        peso,
        ...
        TO_NUMBER(TRIM(v_masa_txt)),
        CASE WHEN v_estatura_txt IS NULL THEN NULL ELSE TO_NUMBER(TRIM(v_estatura_txt)) END,
        TO_NUMBER(TRIM(v_peso_txt)),
        TO_NUMBER(TRIM(v_cliente_txt))
      );
    EXCEPTION
      WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERROR en línea #'||v_line_no||
                             ' fecha="'||v_fecha_txt||'" masa="'||v_masa_txt||
                             '" estatura="'||v_estatura_txt||'" peso="'||v_peso_txt||
                             '" cliente_txt="'||v_cliente_txt||'"');
  ...
  DBMS_OUTPUT.PUT_LINE('Carga registro_medidas OK.');
