-- Inserta en la tabla status_aparato
create or replace procedure carga_status_aparato is
...
begin
  ...
  v_rows := 100;
  v_query := 'insert into status_aparato(DESCRIPCION,NOMBRE_STATUS) values (:ph1,:ph2)';

  -- 2. Abre el archivo en modo lectura
  v_file := UTL_FILE.FOPEN('INFRA_DIR', 'status_aparato.csv', 'R', 32767);
  ...
  loop
    ...
    begin
      ...
      if v_field_count >= 2 then
        ...
        v_nombre_status := TRIM(v_fields(2));

        -- 7. Realizar las inserciones
        execute immediate v_query
        using v_descripcion, v_nombre_status;
      else
    ...
    exception
      when others then
        ...
        if v_field_count >= 2 then
          DBMS_OUTPUT.PUT_LINE('Campo 2 (NOMBRE_STATUS): [' || SUBSTR(v_fields(2), 1, 50) || ']');
        end if;
  ...
  DBMS_OUTPUT.PUT_LINE('Carga de status_aparato completada exitosamente');
END;
/

-- Ejecutar el procedimiento
BEGIN
  carga_status_aparato;
END;
/
