...
-- @Descripción: Carga datos en la tabla SALA_DISCIPLINA desde sala_disciplina.csv

create or replace procedure carga_sala_disciplina is
  v_file   UTL_FILE.FILE_TYPE;
  v_line   VARCHAR2(32767);
  v_is_first_line BOOLEAN := TRUE;

  -- Columnas de la tabla SALA_DISCIPLINA
  v_disciplina_id   NUMBER(6,0);
...
begin
  ...
  dbms_output.put_line('CARGA DE DATOS - TABLA SALA_DISCIPLINA');
  dbms_output.put_line('=============================================================');
  dbms_output.put_line('');

  -- Abrir archivo CSV
  v_file := UTL_FILE.FOPEN('INFRA_DIR', 'sala_disciplina.csv', 'R', 32767);
  ...
  loop
    ...
    begin
      ...
      -- Parsear campos (limpiar espacios)
      v_disciplina_id := TO_NUMBER(TRIM(v_fields(1)));
      v_sala_id       := TO_NUMBER(TRIM(v_fields(2)));

      -- Insertar registro en la tabla SALA_DISCIPLINA
    ...
    exception
      when DUP_VAL_ON_INDEX then
        ...
        dbms_output.put_line('');
        dbms_output.put_line('  ✗ ERROR: Par duplicado (DISCIPLINA_ID, SALA_ID)');
        dbms_output.put_line('    Línea: ' || v_line);
        dbms_output.put_line('    DISCIPLINA_ID: ' || v_disciplina_id);
        dbms_output.put_line('    SALA_ID: ' || v_sala_id);
        dbms_output.put_line('');
      when OTHERS then
        v_errores := v_errores + 1;
        dbms_output.put_line('');
        ...
        if v_fields.EXISTS(1) then
          dbms_output.put_line('    Campo 1 (disciplina_id): [' || v_fields(1) || ']');
        end if;
        if v_fields.EXISTS(2) then
          dbms_output.put_line('    Campo 2 (sala_id): [' || v_fields(2) || ']');
        end if;
        dbms_output.put_line('');
    end;
  end loop;
...
exception
  when OTHERS then
    ...
    raise;
end carga_sala_disciplina;
/

show errors

prompt
prompt Ejecutando carga de datos para tabla SALA_DISCIPLINA...
prompt

set serveroutput on size unlimited
exec carga_sala_disciplina;
